...
contract Strategy is BaseStrategy {
    ...
    function adjustPosition(uint256 _debtOutstanding) internal override {
        ...
        if (want.balanceOf(address(this)) >_debtOutstanding){
            uint256 availableToInvest = want.balanceOf(address(this)).sub(_debtOutstanding);
            externalVault.deposit(availableToInvest);
        }
        ...
        returns (uint256 _liquidatedAmount, uint256 _loss)
    {
        ...
        if(_amountNeeded > balanceOfWant){
            ...
            externalVault.withdraw(Math.min(balanceOfStake, _amountNeeded.sub(balanceOfWant)));
        }
